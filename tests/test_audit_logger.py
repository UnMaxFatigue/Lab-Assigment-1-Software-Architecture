...
def test_audit_logger_log_and_integrity():
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        logfile = os.path.join(td, "audit.csv")
        al = AuditLogger(logfile)
        al.logEvent("EVENT_ONE")
        al.logEvent("EVENT_TWO")
        assert len(al.entries) == 2
        # Check file written while directory still exists
...
def test_rollback_removes_last_entry():
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        logfile = os.path.join(td, "audit2.csv")
        al = AuditLogger(logfile)
        al.logEvent("A")
        al.logEvent("B")
        al.logEvent("C")
