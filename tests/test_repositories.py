...
def test_user_repository_save_load():
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        p = os.path.join(td, "users.csv")
        repo = UserCSVRepository(p)
...
def test_vehicule_repository_save_load():
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        p = os.path.join(td, "vehicules.csv")
        repo = VehiculeCSVRepository(p)
...
def test_rental_repository_save_load():
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        p = os.path.join(td, "rentals.csv")
        repo = RentalCSVRepository(p)
